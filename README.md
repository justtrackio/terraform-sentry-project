...
| <a name="input_base_url"></a> [base\_url](#input\_base\_url) | The target Sentry Base API URL in the format https://[hostname]/api/. The default value is https://sentry.io/api/. The value must be provided when working with Sentry On-Premise. | `string` | n/a | yes |
...
| <a name="input_rate_limit_count"></a> [rate\_limit\_count](#input\_rate\_limit\_count) | Number of events that can be reported within the rate limit window. | `number` | `10` | no |
| <a name="input_rate_limit_window"></a> [rate\_limit\_window](#input\_rate\_limit\_window) | Length of time that will be considered when checking the rate limit. | `number` | `1` | no |
